  ...
  axios.get('/users/getPost')
    .then(function (response) {
      ...
      for (let i = post.length-1; i > -1; i--){
        ...
        <div class="post-title"><h3>${post[i].title}</h3></div>
        <div class="post-user">${post[i].user}发表</div>
        <div class="post-content">${post[i].content}</div>
        <div class="post-comment">
          <div class="comment-title">
            <h3>评论：</h3>
            <a href="javascript:void(0)"  onclick="fabiaopinglun(${post[i].postId})" id="commentPostId${post[i].postId}" class = "tuichudenglu">发表评论</a>
          </div>
      ...
      axios.get('/users/getComment')
        .then(function (response) {
          ...
          for (let i = post.length-1; i > -1; i--) {
            for (let j = comment.length-1; j > -1; j--) {
              if (post[i].postId == comment[j].postId) {
                console.log(comment)
                $(`#postId${comment[j].postId} .comment-content`).append(`<div>${comment[j].user} ：  ${comment[j].content}</div>`)
              }
            }
          }
        })
                ...
                for (let i = post.length-1; i > -1; i--) {
                  var midContentHtml = `
                    ...
                    <div class="post-comment">
                      <div class="comment-title">
                        <h3>评论：</h3>
                        <a href="javascript:void(0)" onclick="fabiaopinglun(${post[i].postId})" id="commentPostId${post[i].postId}" class="pinglun">发表评论</a>
                      </div>
                      <div class="comment-content">
                      </div>
                    </div>
...
function tuichu() {
  $(".userLogin").empty()
  $(".userLogin").append('<a href="javascript:void(0)" id="login_click" class="tuichudenglu" title="嘿，这个登陆不一样了" onclick="btnLogin()">登录</a>')
  isLoginPage = false
                ...
                .then(function (response) {
                  ...
                  axios.get('/users/getComment')
                    .then(function (response) {
                      ...
                      console.log(post.lenth)
                      for (let i = post.lenth-1; i > -1; i--) {
                        for (let j = comment.length-1; j > -1; j--) {
                          if (post[i].postId == comment[j].postId) {
...
////////////////////////////////////////////////////////////////
window.alert("当前服务器流量较小(而且大多是作者自己电脑做服务器)可能加载较慢,如果有不显示的情况请多等一会");